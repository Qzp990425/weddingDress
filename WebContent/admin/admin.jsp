    ...
    <div class="wrapper" style="width:1250px;height:800px;margin-left:-10px;margin-top:-20px">
        ...
        <div class="page-content">
          <div class="content">
            ...
            <xblock><button class="layui-btn layui-btn-danger" onclick="recoverAll()">管理员查询</button></xblock>
            <table class="layui-table" style="text-align:center">
                ...
                <tbody>
                	<c:forEach var="admins" items="${pageInfo.list}">
                      <tr style="height:120px;">	           			  
						  ...
						  <td>
						  	<c:if test="${admins.status==0}">超级管理员</c:if>		
						  	<c:if test="${admins.status==1}">总经理</c:if> 
						  	<c:if test="${admins.status==2}">店长</c:if>		
						  	<c:if test="${admins.status==3}">员工</c:if>		
						  </td>		
	                      <td class="text-center">
	                        <div class="layui-btn-group">
	                        	<c:if test="${admins.status>admin.status}">
	                        	 <a href="${pageContext.request.contextPath}/admin/deleteAdmin?aid=${admins.aid}" class="layui-btn layui-btn-xs layui-btn-danger">删除</a>
	                          </c:if>
	                        </div>
	                      </td>
                      </tr>
